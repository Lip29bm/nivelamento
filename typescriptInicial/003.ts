...
function numerosDeArmstrong(): void {
  let numeroInserido: string = String(
    readline.question(`insira o numero aqui: `)
  );
  ...
  if (resultado == Number(numeroInserido)) {
    console.log(
      `Sim o numero ${Number(numeroInserido)} e um numero de armstrong`
    );
  } else {
    console.log(
      `Nao o numero ${Number(numeroInserido)} nao e um numero de armstrong`
    );
  }
}
numerosDeArmstrong();
